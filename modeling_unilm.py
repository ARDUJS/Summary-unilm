...
from transformers.modeling_albert import AlbertMLMHead, AlbertPreTrainedModel
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None):
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                # print("decoder attention_mask", attention_mask.shape, attention_mask[0][0][0], attention_mask[0][0][1])
                # print("decoder hidden_states", hidden_states.shape)
                # print("decoder history_states", history_states.shape)
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None, masked_pos=None, masked_weights=None, num_tokens_a=None, num_tokens_b=None):
        ...
        sequence_output, __ = self.bert(
            input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)  # batch_size * 512 * 768
...
class UnilmForSeq2Seq_w(AlbertPreTrainedModel):
    """refer to BertForPreTraining"""
    def __init__(self, config):
        super(UnilmForSeq2Seq_w, self).__init__(config)
        ...
        # print(config)
        # print(self.cls)
        # if self.albert.embeddings.token_type_embeddings.num_embeddings < 6:
        #     my_embedding = nn.Embedding(6, config.embedding_size)
        #     my_embedding.weight.data = self.albert.embeddings.token_type_embeddings.weight.data.repeat(3, 1)
        #     self.albert.embeddings.token_type_embeddings = my_embedding
