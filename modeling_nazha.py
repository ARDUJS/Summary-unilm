    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            if history_states is not None:
                hidden_states = torch.cat((history_states, hidden_states), dim=1)
...
class NeZhaAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.self(
            hidden_states, attention_mask, head_mask, encoder_hidden_states, encoder_attention_mask, history_states=history_states
        )
...
class NeZhaLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(hidden_states, attention_mask, head_mask, history_states=history_states)
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                ...
                layer_outputs = layer_module(
                    hidden_states, attention_mask, head_mask[i], encoder_hidden_states, encoder_attention_mask, history_states=history_states
                )
...
class NeZhaForSeq2Seq(NeZhaPreTrainedModel):
    """refer to BertForPreTraining"""
    def __init__(self, config):
        super(NeZhaForSeq2Seq, self).__init__(config)
        self.bert = NeZhaModel(config)
        self.cls = BertOnlyMLMHead(config)
    ...
    def forward(
        ...
        # print(input_ids, input_ids.shape, token_type_ids)
        # print(self.bert.embeddings)
        sequence_output= self.bert(
            input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)[0]  # batch_size * 512 * 768
        ...
        # seqence_output_masked: [32, 20, 768]
        # print("sequence_output_masked", sequence_output_masked.shape)


        # print("sequence_output_masked: ", sequence_output_masked.shape)
        prediction_scores_masked = self.cls(sequence_output_masked)
        # print("prediction_scores_masked", prediction_scores_masked.shape)
        # print("self.crit_mask_lm_smoothed", self.crit_mask_lm_smoothed)
...
class NeZhaModelIncr(NeZhaModel):
    def __init__(self, config):
        super(NeZhaModelIncr, self).__init__(config)

    def forward(self, input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True, prev_embedding=None,
                prev_encoded_layers=None, head_mask=None):
...
class NeZhaForSeq2SeqDecode(NeZhaPreTrainedModel):
    def __init__(self, config, mask_word_id=0,
        ...
        super(NeZhaForSeq2SeqDecode, self).__init__(config)
        self.bert = NeZhaModelIncr(config)
        self.cls = BertOnlyMLMHead(config)
        self.crit_mask_lm = nn.CrossEntropyLoss(reduction='none')
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask):
        ...
        # while next_pos < output_length :
            ...
            # print("curr_token_type_ids", curr_token_type_ids.shape)
            # print("curr_position_ids", curr_position_ids.shape)
            # print("curr_attention_mask", curr_attention_mask.shape)
            ...
            new_embedding, new_encoded_layers, _ = \
                self.bert(x_input_ids, token_type_ids=curr_token_type_ids, attention_mask=curr_attention_mask,
            ...
            # n = 0
            # done_set = set()
            # for i, _id in enumerate(max_ids):
            #     if max_ids[i] in (102, 0) and i not in done_set:
            #         done_set.add(i)
            #         n += 1
            # if n >= max_ids.shape[0]:
            #     break
